...
const GameProvider = ({children, playerId, teamId, position}:IProps) => {
  const [state, dispatch] = useReducer((state: IGameContextState, action: IAction) => {
    switch (action.type) {
      ...
      case 'setLastMessage':
        ...
        switch (message.type) {
          case 'startGame':
            return {...state, lastMessage: action.value, game: message.value};
          default:
  ...
  useEffect(() =>
      sendMessage(JSON.stringify({
        type: 'addPlayer',
        value: {gameId: 'game', playerId, teamId, position}
      }))
    , [sendMessage, playerId, teamId, position]);
